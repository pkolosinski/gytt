...
1. [ ] Audit and complete semantic headings, landmarks, labels, descriptions, and field-error associations across Dashboard, Tasks, and Habits.
2. [ ] Complete visible focus, keyboard operation, and status announcements, retaining the tested non-drag Task Move action.
3. [ ] Complete modal create/edit/delete/convert focus trapping and return focus to each originating trigger.
4. [ ] Complete Habit panel/full-page focus behavior and return focus to the originating Habit card.
5. [ ] Complete keyboard-expandable Completed sections and non-color-only progress/status presentation.
6. [ ] Implement Dashboard phone/desktop containment with no horizontal page overflow.
7. [ ] Implement the bounded horizontally scrollable, snap-aligned three-column phone Tasks board while keeping the page shell fixed.
8. [ ] Complete route-selected wide-panel/narrow-full-page Habit details layout without changing route semantics.
