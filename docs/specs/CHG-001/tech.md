...
- `GET /health/ready` returns `200 { ready: true }` only after configuration, Couchbase, matching migration checksums, image/schema compatibility, disabled usage sharing or the explicitly accepted pinned Community Edition limitation, and a bounded database check succeed; otherwise it returns `503 { ready: false }`.
...
- `SchemaMigrationDocument`: `version: integer`, `name: string`, `checksum: string`, `appliedAt: timestamp`, and `provisioningChecks: { applicationTelemetryDisabled: boolean, usageSharingDisabled: boolean, updateChecksDisabled: boolean, communityEditionPrivacyLimitationsAccepted: boolean } | null`. The initial migration records the observed values and whether the pinned Community Edition limitation was explicitly accepted; runtime readiness requires all settings to be disabled or requires that recorded limitation together with the explicit Community Edition opt-in.
...
- **Security:** HTTPS is mandatory. The reverse proxy verifies one shared Basic credential for the whole origin, stores only its supported salted password hash outside source and images, rate-limits repeated authentication failures, and strips `Authorization` before forwarding. The application and database run without root, and the runtime database identity cannot administer users or cluster configuration. Community Edition does not expose the required collection-scoped application roles, so the runtime identity is bucket-scoped `bucket_full_access`; its broader data scope is an accepted limitation and is tested explicitly.
...
- Couchbase readiness requires initialized services, bucket, scope, collections, indexes, matching migration checksums, a schema version within the running image's declared compatibility range, either disabled usage/update settings or the explicitly accepted pinned Community Edition limitation, and a bounded database check; an open TCP port is insufficient.
- An operator invokes migrations from the GYTT image with one-time Couchbase administrator secrets. The long-running application receives only a bucket-scoped runtime credential that cannot administer users or cluster configuration.
- The Compose data network is marked internal and has no external egress. This preserves the local-only product boundary even though Community Edition reports `sendStats=true` and does not expose the Enterprise application-telemetry endpoint.
...
- `GYTT_COUCHBASE_ALLOW_COMMUNITY_PRIVACY_LIMITATIONS` — explicit `true` opt-in for the pinned Community Edition's unsupported application-telemetry endpoint and mandatory `sendStats=true` behavior; default `false`.
- `GYTT_PROVISION_KV_TIMEOUT_MS`, `GYTT_PROVISION_QUERY_TIMEOUT_MS`, `GYTT_PROVISION_TRANSACTION_TIMEOUT_MS`, `GYTT_PROVISION_READBACK_TIMEOUT_MS`, and `GYTT_PROVISION_HTTP_TIMEOUT_MS` — optional one-shot startup budgets used while Couchbase query and index services initialize.
...
Couchbase Enterprise application telemetry is explicitly disabled and verified during provisioning; the pinned Community Edition returns an Enterprise-only response for that endpoint, which is recorded as not applicable rather than treated as an unverified success. Community Edition 8.0.2 rejects `POST /settings/stats` with `sendStats=false` and requires `sendStats=true`; the provisioning record stores this exact limitation and readiness accepts it only when `GYTT_COUCHBASE_ALLOW_COMMUNITY_PRIVACY_LIMITATIONS=true`. The Compose data network has no external egress, so the setting cannot send usage data outside the local deployment. There are no runtime feature flags.
...
- **Couchbase privacy settings — explicit Community Edition limitation:** provisioning verifies Enterprise application telemetry is disabled when the endpoint exists, records the pinned Community Edition endpoint/`sendStats` limitation when it does not, and blocks readiness unless that limitation is explicitly accepted.
...
Scenario: Block readiness when privacy limitations are not accepted
  Given Couchbase privacy settings cannot be verified disabled and the pinned Community Edition limitation is absent or not explicitly accepted
...
- **Persistence seam:** Testcontainers runs the pinned Couchbase image and real provisioning, records its Community Edition privacy limitation, verifies the explicit privacy opt-in and internal no-egress network boundary, forward migration checksums, schema compatibility gates, filtered indexes, consumed-ID receipts, the board-composition revision, parameterized SQL++ with hostile values, `REQUEST_PLUS` immediate reads, CAS conflicts, Habit lifecycle/progress races, conversion transactions, fenced board reads racing conversion, ambiguous non-transaction and transaction read-back, telemetry-readiness failure, and restart with the same volume.
